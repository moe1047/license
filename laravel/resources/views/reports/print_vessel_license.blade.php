...
@section('content')
    <div class="row">
        <div class="col-md-12">
            {!! Form::open(['url' => 'admin/report/printVessel','method'=>'get']) !!}
            ...
            <div class="box">
                ...
                <div class="box-body " id="table-wrapper">
                    <table class="table table-bordered datatable">
                        <thead>
                        <tr>
                            <th>License</th><th>Vessel Name</th><th>Nationality</th><th>Renewed date</th><th>Expire Date</th><th>Print</th>
                        </tr>
                        </thead>
                        <tfoot>
                        <tr>
                        ...
                        @if(isset($renewals))
                            @foreach($renewals as $renewal)
                                <tr>
                                    <td>{{$renewal->vesselLicense->licenseNo}}</td><td>{{$renewal->vesselLicense->vesselName}}</td><td>{{$renewal->vesselLicense->nationality}}</td>
                                    <td>{{$renewal->date}}</td><td>{{$renewal->expire_date}}</td>
                                    <td><a href="{{url('admin/report/'.$renewal->id.'/printVessel')}}" target="_blank" class="btn btn-primary btn-xs">Print</a></td>
                                </tr>
                            @endforeach
